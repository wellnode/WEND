...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected WEND and zWEND supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zWENDsupply']:
                assert_equal(node['zWENDsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check WEND and zWEND supply at the beginning
        # ------------------------------------------
        # zWEND supply: 2 coins for each denomination
        ...
        # WEND supply: block rewards minus burned fees for minting
        ...
        # Verify that WEND and zWEND supplies were properly updated after the spends and reorgs
        self.log.info("Check WEND and zWEND supply...")
