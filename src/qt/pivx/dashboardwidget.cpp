    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of WEND earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 WEND");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("WEND")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair WEND, zWEND
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(108, 188, 179));
	set1->setColor(QColor(15, 94, 146));
