...
const char * const PIVX_CONF_FILENAME = "wend.conf";
const char * const PIVX_PID_FILENAME = "wend.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\wend
// Windows >= Vista: C:\Users\Username\AppData\Roaming\wend
// Mac: ~/Library/Application Support/wend
// Unix: ~/.wend
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wend";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\wendParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\wendParams
    // Mac: ~/Library/Application Support/wendParams
    // Unix: ~/.wend-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WendParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "WendParams";
#else
    // Unix
    return pathRet / ".wend-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty wend.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Wend config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override wend.conf
