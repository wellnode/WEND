...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all WEND specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:50312"));
...
/** Sanity checks
 *  Ensure that Wend is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Wend is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single Wend process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Wend is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //Wend specific: zerocoin and spork DB's
                ...
                // Wend: load previous sessions sporks if we have them.
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zWEND if already reindexed
