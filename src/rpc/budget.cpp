...
UniValue preparebudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"WEND-address\" monthy-payment\n"
            ...
            "5. \"WEND-address\":   (string, required) WEND address to send payments to\n"
    ...
    if (!pwalletMain->CreateBudgetFeeTX(wtx, proposalHash, keyChange, false)) { // 50 WEND collateral for proposal
...
UniValue submitbudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"WEND-address\" monthly-payment \"fee-tx\"\n"
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) WEND address of payment\n"
