...
{
    // double check that there are no double spent zWEND spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zWEND serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zWEND spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zWEND check to not include duplicated serials in the same block.
