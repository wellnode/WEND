...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 403200)	return 5 * COIN;
    if (nHeight > 201600)	return 8 * COIN;
    if (nHeight > 57600)	return 10 * COIN;
    if (nHeight > 43200)	return 8 * COIN;
    if (nHeight > 36000)	return 5 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 WEND tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 WEND tx -> 1 confirmation
