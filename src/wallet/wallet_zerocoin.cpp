...
bool CWallet::AddDeterministicSeed(const uint256& seed)
{
    ...
    if(IsCrypted()) {
        ...
        strErr = "save zWENDseed to wallet";
    }
    //the use case for this is no password set seed, mint dzWEND,
...
{
    //If this is a zapwallettx, need to read zWEND
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zWEND output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to WEND
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zWEND was successful
        ...
        CTxDestination * changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zWEND"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zWEND public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zWEND used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        ...
        // Select the zWEND mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to WEND address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zWEND in the future, for now it's useless,
